...
State storage, i.e., `State<T>`, is implemented as a controllable Arena GC. It performs mark-and-sweep garbage collection every frame to destroy state that is no longer in use. You can refer to [gc-arena](https://github.com/kyren/gc-arena) for similar concepts.
